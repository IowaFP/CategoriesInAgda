...
module YonedaLemma (𝒞 : Category o a e) where 
  open Category 𝒞
  private 
    𝒞ᵒᵖ = op 
    variable 
  ...
  module CovariantYoneda (F : 𝒞 ⇛ 𝐒𝐞𝐭𝐨𝐢𝐝 a e) where
    open Functor F ; open _⇉_ 
    import Relation.Binary.Reasoning.Setoid as SetoidReasoning

    -- Mapping natural transformations to terms in F A 
    Yoneda→ : ∀ A → Nat(Hom[ A ,—] , F) ⇉ F₀ A
    Yoneda→ A .smap (η , η-nat) = η {A} · Id
    Yoneda→ A .hom f = f Id

    -- Mapping terms in F A to natural transformations
    ←Yoneda : ∀ A → F₀ A ⇉ Nat(Hom[ A ,—] , F)
    ←Yoneda A .smap X = ε , natural
      where    
        ε : ∀ {B} → Hom(A , B) ⇉ F₀ B 
    ...
    ←Yoneda A .hom x≈y f = fmap f .hom x≈y 
  ...
  module ContravariantYoneda (F : 𝒞ᵒᵖ ⇛ 𝐒𝐞𝐭𝐨𝐢𝐝 a e) where
    open Functor F        
    
    Yonedaᵒᵖ→ : ∀ A → Nat(Hom[—, A ] , F) ⇉ F₀ A
    Yonedaᵒᵖ→ = TODO 

    ←Yonedaᵒᵖ : ∀ A → F₀ A ⇉ Nat(Hom[—, A ] ,  F)
    ←Yonedaᵒᵖ = TODO 
  ...
  module YonedaEmbedding₁ where
    ...
    -- One way to define the Yoneda embedding is to describe 𝒞ᵒᵖ as a
    --  full sub-category of [ 𝒞 , 𝐒𝐞𝐭𝐨𝐢𝐝 a e ]
    𝒞ᵒᵖ′ : Category _ _ _
    𝒞ᵒᵖ′ = FullSubcategory ([ 𝒞 , 𝐒𝐞𝐭𝐨𝐢𝐝 a e ]) {_} Obj λ A → Hom[ A ,—] 
    open _↪_ 

    𝓎↪ : 𝒞ᵒᵖ′ ↪ [ 𝒞 , 𝐒𝐞𝐭𝐨𝐢𝐝 a e ]
    𝓎↪ = Subcategory↪ ([ 𝒞 , 𝐒𝐞𝐭𝐨𝐢𝐝 a e ]) {_} {Obj} λ A → Hom[ A ,—]

    𝓎 : 𝒞ᵒᵖ′ ⇛ [ 𝒞 , 𝐒𝐞𝐭𝐨𝐢𝐝 a e ]
    𝓎 = 𝓎↪ .ι

    -- We prove that 𝒞ᵒᵖ and 𝒞ᵒᵖ′ are equivalent categories
    open Definition2
    equivalent : areEquivalent 𝒞ᵒᵖ 𝒞ᵒᵖ′ 
    equivalent = TODO
